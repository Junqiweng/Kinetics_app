...
- ✅ **V1.66**: `modules/` 与 `app.py` 联动重构（新增上下文构建、拟合页拆分、配置导出公共模块，零行为变化）
- ✅ **V1.65**: 配置验证逻辑优化（允许 `K0_ads = 0`，添加非负值检查）
- ✅ **V1.64**: 可逆反应边界设置，支持可逆反应参数拟合
- ✅ **V1.63**: 增强配置管理和异常处理，会话清理优化
- ✅ **V1.62**: 常量集中管理（`constants.py`），持久化目录统一管理
- ✅ **V1.61**: 数值稳定性提升，刷新保留配置功能
- ✅ **V1.60**: 有效数字显示改进
- ✅ **V1.59**: 全局 CSS 样式和 Matplotlib 绘图样式
- ✅ **V1.58**: 摩尔分数输出选项，配置导入/导出功能
...
- **PFR**：假设恒定体积流量（$\dot{v} = \text{const}$），即 $C_i = F_i / \dot{v}$
  - 不适用于气相反应（摩尔数变化导致 $\dot{v}$ 变化）
  - 不考虑压降
