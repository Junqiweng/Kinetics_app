...
### C2. `初值与基础边界`中的 L-H 边界（仅当动力学为 `langmuir_hinshelwood`）
...
- 若启用 `K₀,ads 用 log 拟合` 且勾选了 `拟合 K₀,ads`，则对应初值和 `K0_ads Min/Max` 都必须严格大于 0
- `Ea_K` 允许负；若数据温度跨度小，建议先固定

### C3. `1. 可逆反应边界设置（逆反应）`（仅当启用 `reversible_enabled`）
...
#### (3) `k₀ 用 log 拟合`、`k₀,rev 用 log 拟合`、`K₀,ads 用 log 拟合`

- 含义：优化器在 log 空间搜索这些正参数，但模型计算时仍使用线性空间参数
- 当前默认：3 个开关都默认启用
- 适用原因：
  - `k₀`、`k₀,rev` 往往跨多个数量级，log 拟合通常更稳
  - `K₀,ads` 在 L-H 模型下也常跨数量级，因此同样适合 log 拟合

使用注意：

- 启用 log 拟合后，对应“被选中拟合”的参数初值必须 > 0
- 对应边界下限也必须 > 0
- 如果某个参数你希望允许取 0，就不要对它启用 log 拟合，或不要勾选该项参与拟合

#### (4) `最大步长比例（max_step_fraction）`（限制 ODE 积分步长）
...
#### (5) `多起点搜索（Multi-start）`、`起点数量（n_starts）`、`粗拟合迭代上限（max_nfev_coarse）`
