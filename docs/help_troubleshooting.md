...
1) 更推荐：回到「③ 拟合与结果」→ `初值与基础边界` 把初值改到边界内（把初值放到区间中部通常更稳）
2) 或在同一区域放宽边界以包含初值
3) 再开始拟合
...
1) 检查「③ 拟合与结果」→ `初值与基础边界` 的 `k₀ / Eₐ` 表是否存在空白、`1,000`、`<0.1`、文本
...
Windows 下使用 `LSODA` 时，拟合会自动禁用行级并行，以避免 ODEPACK 并发重入造成“很快收敛但拟合很差”。如果需要更高速度，可把求解器切换到 `BDF` 或 `Radau` 对比。
...
这是当前版本新增的保护提示。只要你修改了数据、物种、$\nu$、输出模式、求解器、残差类型或 `max_step_fraction` 等关键输入，结果页就会提示旧结果已过期。

解决：

1) 查看提示下方列出的变化原因
2) 重新点击 `🚀 开始拟合`
3) 如需彻底开始新任务，可用侧边栏 `新建项目`
