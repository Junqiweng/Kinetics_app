...
| `x0 infeasible`（初值越界） | [B1](#b1-x0-infeasible初值越界) |
...
### B1. `x0 infeasible`（初值越界）

含义：你勾选拟合的某些参数初值不在边界内（`k0 Min/Max`、`Ea Min/Max`、`Order Min/Max` 等），导致优化器无法开始。

典型触发方式：

- 勾选了 `拟合 k₀`，但 `k₀` 初值不满足 `k0 Min ≤ k₀ ≤ k0 Max`
...
- 容差过严（`rtol/atol` 太小）
- 步长控制不合适（`max_step_fraction (ODE)`）
- 参数边界太宽，优化跑到极端区域（导致速率极快/极慢）

处理（按优先级）：

1) 侧边栏把 `Method` 换成 `BDF`（最常用稳健选项）或 `Radau`
2) 放宽容差：把 `rtol/atol` 调大一档（例如 `rtol: 1e-6 → 1e-5`，`atol: 1e-9 → 1e-8`）
3) 在「③」把 `max_step_fraction (ODE)` 调小（更稳但更慢），或设 `0`（不限制）做对比
...
3) 开启 `Multi-start (多起点)`（`Start Points (起点数)` 设 `5~20`；`Coarse Iters (粗拟合)` 用默认起步）
