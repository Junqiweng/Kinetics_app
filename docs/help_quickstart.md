...
- 如需提速，可在 `从反应方程式生成（可选）` 中直接输入 `A -> B`

2) **数据：② 实验数据**

- 上传 CSV（或下载 `CSV 模板` 后填入）
- 选择 `拟合目标变量` 与“进入目标函数的物种”（即输出物种）
- 上传后可直接在 `数据预览（可直接编辑）` 中修正数据
- 查看 `数据质量检查`，确认必需列、测量列和“当前可拟合行数”正常
- 若为 PFR，先确认侧边栏 `PFR 流动模型` 与 CSV 输入列匹配（液相 `vdot_m3_s` / 气相 `P_Pa`）

3) **拟合：③ 拟合与结果**

- 先在 `初值与基础边界` 中设置参数初值与基础边界
- 再按需展开 `高级设置 (点击展开)` 调 `diff_step`、`Multi-start`、残差类型等
- 点击 `🚀 开始拟合`
...
- `从反应方程式生成（可选）`：按 `A -> B`、`A + B -> C` 等格式快速生成 $\nu$

### 1.3 「③ 拟合与结果」→ `初值与基础边界`

这是当前版本中“参数初值 + 是否拟合 + 基础边界”的核心区域：

- `k₀`、`Eₐ [J/mol]`（按反应）
- `反应级数 n`（行=反应，列=物种）
- 若动力学为 `langmuir_hinshelwood`：`K₀,ads`、`Eₐ,K [J/mol]`、`m`
- 若启用 `reversible_enabled`：`k₀⁻`、`Eₐ⁻ [J/mol]`、`n⁻`
- 基础边界：`k₀ Min/Max`、`Eₐ Min/Max`、`Order Min/Max`
- 若动力学为 `langmuir_hinshelwood`：L-H 边界 `K0_ads Min/Max`、`Ea_K Min/Max` 也在这里，不再放在“高级设置”里
...
- 算法与鲁棒性：
  ...
  - `启用雅可比尺度归一（x_scale='jac'）`、`k₀ 用 log 拟合`、`k₀,rev 用 log 拟合`、`K₀,ads 用 log 拟合`、`随机种子（random_seed）`
...
在「③ 拟合与结果」→ `初值与基础边界` 中设置的 `k₀ Min/Max`、`Eₐ Min/Max`、`Order Min/Max` 只对**勾选拟合**的参数生效。
...
| 正向 `k₀/Eₐ` 初值 + 拟合勾选 | `k₀`、`Eₐ [J/mol]`、`拟合 k₀`、`拟合 Eₐ` | 「③ 拟合与结果」→ `初值与基础边界` | `Eₐ` 单位 J/mol |
| 级数矩阵 `n` + 拟合勾选 | `n_<物种>`、`拟合 <物种>` | 「③ 拟合与结果」→ `初值与基础边界` | 行=反应，列=物种 |
| 参数边界 | `k₀ Min/Max`、`Eₐ Min/Max`、`Order Min/Max` | 「③ 拟合与结果」→ `初值与基础边界` | 强烈建议先设 |
| 数值差分步长 | `差分步长（diff_step）` | 「③ 拟合与结果」→ `高级设置` | 相对步长；拟合抖动/不动常调它 |
| ODE 步长限制 | `最大步长比例（max_step_fraction）` | 「③ 拟合与结果」→ `高级设置` | `0` 表示不限制 |
| 多起点 | `多起点搜索（Multi-start）`、`起点数量（n_starts）` | 「③ 拟合与结果」→ `高级设置` | 参数多/易局部极小值时开启 |
