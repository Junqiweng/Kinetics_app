...
- PFR 入口（拟合目标为 `Fout/xout`）：`F0_<物种名>_mol_s`
- PFR 入口（拟合目标为 `Cout`）：`C0_<物种名>_mol_m3`（App 会用 `F0 = C0 × vdot` 自动换算参与计算）
...
> 最常见错误：列名写 `vdot_m3_s` 但数据填了 L/min；或列名写 `t_s` 但数据填了分钟。结果不会报错（因为是数值），但物理上会完全错误。
