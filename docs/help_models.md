...
- 逆向参数区位于「③ 拟合与结果」→ `初值与基础边界` 的 `可逆反应 (逆反应) 参数` 展开框（需先启用可逆开关）
...
- **液相(恒定 $\dot v$，默认)**：$C_i = F_i/\dot v$（不含体积流量沿程变化）
...
- 勾选拟合前，先去「③」设置边界：`k₀ Min/Max`、`Eₐ Min/Max`，并确保初值在边界内（避免 `x₀ infeasible`）
...
位置：`③ 拟合与结果` → `初值与基础边界` → `Langmuir-Hinshelwood (L-H) 参数`
...
> 说明：默认边界来自 App 内置常量（用于「③ 拟合与结果」→ `初值与基础边界` 与 `高级设置` 的初始值）。实际项目建议根据体系收紧。

| 参数（UI 名称） | 维度 | 单位 | 在哪里改 | 默认边界（Min/Max） | 常见坑 |
|---|---:|---|---|---|---|
| `k₀` | 按反应 | \[SI]（依赖总级数） | `①` 参数表 | `k₀ Min=1e-15`，`k₀ Max=1e15` | 初值越界导致 `x₀ infeasible`；输入非数字→`NaN` |
| `Eₐ [J/mol]` | 按反应 | J/mol | `①` 参数表 | `Eₐ Min=3e4`，`Eₐ Max=3e5` | 温度跨度不足时不可辨识；与 `k₀` 强相关 |
...
### H2. `x₀ infeasible`（初值越界）

原因：勾选拟合的参数初值不在「③」`初值与基础边界` 中设置的 `k₀ Min/Max`、`Eₐ Min/Max`、`Order Min/Max` 内。
